...
* **契約日数:**
    * **計算:** `scheduled_termination_date` から `contract_date` を引いて算出する。
...
*   **契約日数**: `scheduled_termination_date` と `contract_date` の差から計算されます。日付が不正な場合は計算をスキップし、`null`または`-`として表示されます。
